...
export default async function LeadsPage({
  ...
  const leads = await leadRepo.list({
    ...
    companyContains: company || undefined,
  });

  return (
    <div className="flex flex-col gap-4">
      <h1 className="text-lg font-semibold text-[--color-fg]">Leads</h1>
      <LeadsFilterBar stage={stage} hasEmail={hasEmail} company={company} />
      <LeadsTable leads={leads} />
      {leads.length === 0 ? <p className="text-sm text-[--color-muted]">No leads yet.</p> : null}
    </div>
  );
}
