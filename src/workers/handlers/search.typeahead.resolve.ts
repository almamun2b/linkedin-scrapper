 ...
 * reusable as-is.
 */
export async function handleSearchTypeaheadResolve(
  job: ClaimedJob,
  signal: AbortSignal,
): Promise<void> {
  const parsed = searchTypeaheadResolvePayloadSchema.safeParse(job.payload);
  if (!parsed.success) {
    throw new FatalError("Invalid search.typeahead.resolve payload", {
  ...
  const lockResult = await withAccountLock(linkedInAccountId, () =>
    resolve(linkedInAccountId, kind, query, signal),
  );
  if (!lockResult.ok) {
...
async function resolve(
  ...
  try {
    ...
    log.warn(
      { kind, query },
      "search.typeahead.resolve endpoint not yet implemented — see file header",
    );
    const results: { label: string; urn: string }[] = [];
